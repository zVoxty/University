...
void AnimatedSprite::SetFrame(int iIndex)
{
	...
	mptFrameCrop.x = mptFrameStartCrop.x + (iIndex % (this->width()/this->miFrameWidth))*miFrameWidth;
	mptFrameCrop.y = mptFrameStartCrop.y + (iIndex / (this->height()/this->miFrameHeight))*miFrameHeight;
}
